...
public class ScreenshotUtil {
  ...
  private static BufferedImage createRobotScreenshot() {
      try {
          ...
          LOG.error("Error creating robot screenshot", e);
          return null;
      }
  }
  ...
  private static BufferedImage createDbusScreenshot() {
      try {
                  ...
                  if (path.equals(t.getPath())) {
                      try {
                          ...
                          if (response.intValue() == 0) {
                              ...
                              LOG.error("Failed: response=%s%n", response);
          ...
          LOG.debug("result: %s%n", result);
          LOG.debug("expected path: %s%n", path);
